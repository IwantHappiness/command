...
`git config` - утилита для просмотра и настройки всех аспектах для работы с git. #GitConfig  
 ├── `git config --list` - проверка используемой конфигурации, показывает все настройки git. #GitShowConfig  
...
`git status` - определяет какие файлы в каком состоянии. #GitStatus  
 └── `git status -s` - выводит более краткую информацию. #GitStatusShort  

`git rm` _<Файл или папка>_ - удалит его из индекса и из каталога, принимает glob шаблоны. #GitRm  
 ├── `git rm -f` _<Файл или папка>_ - принудительное удаление.  
 └── `git rm --cached` _<Файл или папка>_  - удаляет из индекса но сохраняет файл в каталоге. #GitRmCached  

---
## Коммит изменений 

`git commit` -  откроет выбранный вами текстовый редактор. #GitCreateCommit  
 ├── `git commit -m` _<Название коммита>_ - фиксация изменений. #GitCreateCommitShort  
 └── `git commit -v`  - в комментарий будет также помещена дельта/diff изменений. #GitCreateCommitDetailed  
